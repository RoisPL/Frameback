...
2. Create a new userscript, remove the example code and paste in the complete
   Frameback code from the `.user.js` file. Save it, then reload Autodarts.
3. Allow the browser to use your camera.
4. Open **Settings → Board image**, choose a camera and start alignment.
5. Freeze the image and mark five points in this order: the centre of the bull,
...
| --- | --- |
| `J` | Open settings |
| `R` | Replay the latest moments |
| `Shift` + `R` | Save the latest moments without playing them |
| `K` | Show or hide the camera view |
| `Shift` + `N` | Turn recording on or off |
| `M` | Switch game mode: Automatic / Shared board / Online |
| `I` | Show or hide the game panel |
| `Z` | Turn automatic zoom on or off |
| `G` | Show the helper grid |
...
2. Utwórz nowy skrypt, usuń przykładową treść i wklej cały kod Frameback z pliku
   `.user.js`. Zapisz skrypt i odśwież Autodarts.
3. Zezwól przeglądarce na dostęp do kamery.
4. Otwórz **Ustawienia → Obraz tarczy**, wybierz kamerę i rozpocznij dopasowanie.
5. Zatrzymaj obraz i zaznacz pięć punktów w tej kolejności: środek bulla, a
...
| `K` | Pokaż lub ukryj podgląd kamery |
| `Shift` + `N` | Włącz lub wyłącz nagrywanie |
| `M` | Zmień tryb gry: Automatycznie / Wspólna tarcza / Gra online |
| `I` | Pokaż lub ukryj panel gry |
| `Z` | Włącz lub wyłącz automatyczny zoom |
| `G` | Pokaż siatkę pomocniczą |
