...
(function () {
  ...
  const TEKSTY = {
    "en": {
        "Na chalkboardzie trzy duże kadry pokazują okolice rzutów pod punktami rywala, aż do dołu okna. Kliknij kadr, aby zobaczyć całą tarczę. Jeśli nie rozpoznam miejsca, zdjęcia pojawią się z boku.": "On the chalkboard, three large frames show the area around each dart. Click a frame to see the full board. Frame size fits the chalkboard; the size slider applies to other layouts. If the area cannot be identified, snapshots appear on the side.",
        "Na chalkboardzie przeciwnika": "On opponent’s chalkboard",

        "Ułatwienia dla touchscreenów": "Touchscreen helpers",
        "Pływający przycisk kamery": "Floating camera button",
        "Pokazuje przy tarczy przycisk, którym można szybko ukryć lub pokazać obraz kamery podczas poprawiania wyniku.": "Shows a button by the board for quickly hiding or showing the camera image while correcting a score.",
        "To samo, co skrót K, tylko bez klawiatury. Ukrywa wyłącznie obraz — kamera, nagrywanie i kalibracja pracują dalej.": "The same as the K shortcut, without a keyboard. It hides the image only — the camera, recording and calibration keep running.",
        "Ukryj obraz kamery": "Hide camera image",
        "Pokaż obraz kamery": "Show camera image",
        "Zbliżenie zdjęć": "Snapshot zoom",
        "Zbliżenie 1× to cała tarcza w kadrze; wyżej kadr podjeżdża do lotki. Na chalkboardzie kadry same wypełniają kolumnę rywala aż do dołu okna, więc suwak rozmiaru dotyczy układu dolnego i bocznego.": "At 1× the whole board fits the frame; higher values move in towards the dart. On the chalkboard the frames fill the opponent’s column down to the bottom of the window, so the size slider applies to the bottom and side layouts.",
                "Dopasowanie punktowe wyłączone: {a}. Bez niego kadry celują po ręcznie ustawionym kole tarczy. Wykonaj dopasowanie ponownie.": "Point alignment turned off: {a}. Without it the frames aim using the manually set board circle. Please align the image again.",
        "Kadr celuje w lotkę według koła tarczy z kalibracji. Włącz siatkę pomocniczą: jeśli narysowany okrąg nie pokrywa się z zewnętrzną krawędzią podwójnego pierścienia, kadry będą mijać lotkę.": "The frame aims at the dart using the board circle from calibration. Turn on the helper grid: if the drawn circle does not sit on the outer edge of the double ring, the frames will miss the dart.",
        "Dopasowanie pięciu punktów ustawia to koło samo i dokładnie. Suwak jest dla obrazu bez dopasowania punktowego.": "Five-point alignment sets this circle by itself and exactly. The slider is for images without point alignment.",
        "Zmieniono proporcje obrazu kamery": "the camera image ratio changed",
        "Dopasowanie zapisano dla innej kamery": "the alignment was saved for another camera",
        "Zmieniono proporcje kamery — wykonaj kalibrację ponownie": "Camera ratio changed — please calibrate again",
        "Inna kamera — wykonaj dopasowanie obrazu": "Different camera — please align the image",

        "Rozmiar zdjęć": "Snapshot size",
        "Przeciągnij uchwyt nad zdjęciami, aby przesunąć pasek. Po kliknięciu uchwytu możesz też użyć strzałek.": "Drag the handle above the snapshots to move the strip. You can also click the handle and use the arrow keys.",
        "Przywróć położenie zdjęć": "Reset snapshot position",
        "Przesuń zdjęcia": "Move snapshots",
        "Przeciągnij lub użyj strzałek": "Drag or use the arrow keys",

        "Widok tarczy": "Board view",
        "Na żywo": "Live",
        "Trzy rzuty": "Three throws",
        "Na żywo + rzuty": "Live + throws",
        "Położenie zdjęć": "Snapshot position",
        "Zdjęcia na dole": "Snapshots at the bottom",
        "Zdjęcia z boku": "Snapshots on the side",
        "Trzy rzuty zachowują zdjęcia po kolejnych lotkach z wybranej kamery. Kliknij zdjęcie, aby je powiększyć. Nowa kolejka czyści zdjęcia.": "Three throws keeps a snapshot after each dart from your selected camera. Click a snapshot to enlarge it. Snapshots clear when a new turn starts.",
        "Zoom po dwóch T20": "Zoom after two T20s",
        "Po dwóch T20 w tej samej kolejce przybliża T20 przed trzecią lotką. Jeśli możesz zamknąć lega, pierwszeństwo ma pole na zamknięcie.": "After two T20s in the same turn, zooms in on T20 before the third dart. If a checkout is available, the finishing target takes priority.",
        "Zdjęcie rzutu {a}": "Throw {a} snapshot",
        "Rzut {a}: {b}": "Throw {a}: {b}",
        "Zamknij · Esc": "Close · Esc",
        "Trzy ostatnie rzuty": "Three throws in this turn",
        "Oczekuję na rzut": "Waiting for a throw",
        "Brak zdjęcia": "No snapshot",
  ...
  function analizujWS(dane) {
    ...
    try { obsluzRzutyWS(kanal, tresc, temat); } catch (e) {}
  ...
  const DEF = {
    ...
    // zoom
    zoomDwaT20: true, _migr111: true, widokRzutow: 'live', rzutyPozycja: 'bottom',
    rzutyRozmiar: 200, _migr113: true, _migr116: true, rzutyZoom: 2.2,
    rzutyX: -1, rzutyY: -1,
  ...
  function load() {
    try {
      ...
      if (r) {
        ...
        if(raw._migr113!==true){c.rzutyRozmiar=Math.max(140,Number(c.rzutyRozmiar)||140);c._migr113=true;}
        // 1.1.6: wieksze okienka i zblizenie przy lotce zamiast calej tarczy.
        if(raw._migr116!==true){c.rzutyRozmiar=Math.max(200,Number(c.rzutyRozmiar)||200);c.rzutyZoom=Number(c.rzutyZoom)||2.2;c._migr116=true;}
        if(raw._migr111!==true){c.zoomDwaT20=true;c._migr111=true;}
  ...
  // Plywajacy przycisk kamery dla ekranow dotykowych (opcja przyciskKamery).
  let przyciskKamery = null;
  // Ostatni powod, dla ktorego pasek nie zadokowal sie przy karcie rywala.
  let dokPowod = 'ok';
  ...
  // Powód, dla którego dopasowanie pięciopunktowe zostało wyłączone samo.
  // Komunikat znikający po kilku sekundach nie wystarcza: w dzienniku sesji
  // 1.1.2 widać było tylko punktowa.enabled=false, bez śladu dlaczego.
  let powodKalibracji = "";
  // deviceId kamery, ktora faktycznie gra - do porownania z kalibracja.
  let kameraDeviceId = "";
  ...
  function stanLinia() {
    ...
    return [
      ...
      `faza=${replayPhase}`, `ukryty=${powodUkrycia}`, `dok=${dokPowod}`,
    ].join("  ");
  }
  ...
  // ---------- interfejs ----------
  // Stan bieżących rzutów i maksymalnie trzy lokalne zdjęcia. Bez zapisu na dysk.
  const widokRzutow={key:'',rzuty:[],zdjecia:[null,null,null],timer:null,epoch:0,rev:0,wyjmowanie:false,sygnal:null};
  let pasekRzutow=null,podgladRzutu=null,rysRzutyKlucz='',wyborWidokuPanel=null;
  function trybWidoku(){return ['live','shots','both'].includes(cal.widokRzutow)?cal.widokRzutow:'live';}
  function anulujZdjecie(){clearTimeout(widokRzutow.timer);widokRzutow.timer=null;widokRzutow.epoch++;}
  function wyczyscZdjecia(){anulujZdjecie();widokRzutow.zdjecia=[null,null,null];widokRzutow.rev++;zamknijZdjecie();}
  function resetRzutow(){wyczyscZdjecia();widokRzutow.key='';widokRzutow.rzuty=[];widokRzutow.wyjmowanie=false;widokRzutow.sygnal=null;}
  function ustawWidokRzutow(mode){
    if(!['live','shots','both'].includes(mode))return;
    cal.widokRzutow=mode;if(mode==='live')wyczyscZdjecia();save();rysRzutyKlucz='';odswiezRzuty();
  }
  function lokalneRzuty(){return daneSwieze()&&(!czyOnline()||mojIndeks()===st.player)&&!bullOff();}
  function nazwaRzutu(r){
    const x=r?.segment;
    if(x&&typeof x.name==='string')return x.name;
    if(x&&Number.isInteger(x.number)&&[1,2,3].includes(x.multiplier))return ['','S','D','T'][x.multiplier]+x.number;
    return '?';
  }
  function obsluzRzutyWS(kanal,d,temat){
    if(!d||typeof d!=='object')return;
    if(kanal==='autodarts.boards'){
      const board=idMojejTarczy();
      if(board&&String(temat||'').split('.')[0]===board){
        if(d.event==='Takeout started'){widokRzutow.wyjmowanie=true;anulujZdjecie();}
        if(['Takeout finished','Manual reset','Started'].includes(d.event))widokRzutow.wyjmowanie=false;
      }
      return;
    }
    if(kanal!=='autodarts.matches')return;
    const id=matchIdZUrl();if(!id||!st.matchId||st.matchId!==id)return;
    const topic=String(temat||'').split('.')[0];if(topic&&!tenSamMecz(topic,id))return;
    if(d.event){
      if(d.event==='throw'&&d.body?.segment)widokRzutow.sygnal={at:Date.now(),label:nazwaRzutu(d.body)};
      if(['turn_start','game_on','delete'].includes(d.event)){resetRzutow();}
      return;
    }
    if(d.error||d.type==='error'||!Array.isArray(d.players)||!Array.isArray(d.gameScores))return;
    if(!Number.isInteger(d.player)||!d.players[d.player]?.id)return;
    if(!lokalneRzuty()){resetRzutow();return;}
    const player=d.players[d.player];
    const turns=Array.isArray(d.turns)?d.turns.filter(x=>x&&x.playerId===player?.id):[];
    // Nie odgadujemy ostatniej kolejki z całej historii meczu.
    if(turns.length!==1||!turns[0].id||!Array.isArray(turns[0].throws)){resetRzutow();return;}
    const turn=turns[0], key=[id,d.set,d.leg,d.player,turn.id].join('|');
    const items=turn.throws.slice(0,3).map((x,i)=>({id:String(x.id||x.createdAt||i),label:nazwaRzutu(x),coords:x.coords||null}));
    const same=widokRzutow.key===key, previous=same?widokRzutow.rzuty:[];
    const signal=widokRzutow.sygnal,received=signal&&Date.now()-signal.at<1200&&signal.label===items.at(-1)?.label;
    const pending=!!widokRzutow.timer;
    if(!same){resetRzutow();widokRzutow.key=key;}
    const changed=JSON.stringify(previous)!==JSON.stringify(items);
    if(!changed)return;
    anulujZdjecie();zamknijZdjecie();
    // Korekta etykiety nie zmienia zdjęcia tej samej lotki; usunięcie lotki
    // usuwa ją oraz kolejne zdjęcia, które mogłyby sugerować nieprawdziwy stan.
    let invalid=false;
    for(let i=0;i<3;i++){
      if(!items[i]||previous[i]?.id!==items[i].id)invalid=true;
      if(invalid)widokRzutow.zdjecia[i]=null;
    }
    widokRzutow.rzuty=items;widokRzutow.rev++;
    const created=Date.parse(turn.throws[items.length-1]?.createdAt);
    const recent=Number.isFinite(created)&&Date.now()-created>=-500&&Date.now()-created<1500;
    const fresh=(items.length===previous.length+1&&(same||recent||received))||
      (pending&&same&&items.length===previous.length&&items.every((x,i)=>x.id===previous[i].id));
    // Po wejściu w środek kolejki nie odtwarzamy zdjęć, których nie widzieliśmy.
    if(fresh&&trybWidoku()!=='live'&&!inReplay&&!blokadaZdjecia(items.length)&&items.length){
      const n=items.length-1,epoch=widokRzutow.epoch,camera=video?.srcObject;
      widokRzutow.timer=setTimeout(()=>{
        widokRzutow.timer=null;
        if(epoch!==widokRzutow.epoch||camera!==video?.srcObject||!lokalneRzuty()||blokadaZdjecia(items.length)||inReplay||!camOk||!visible||trybWidoku()==='live')return;
        if(st.numThrows!==null&&st.numThrows>items.length)return;
        if(!video||video.readyState<2||!video.videoWidth||!video.videoHeight||!curSize)return;
        try{
          ...
          const style=wygladZdjecia();
          const coords=items[n].coords,rad=typeof promienPol==='function'?promienPol():.94;
          const focus=coords&&Number.isFinite(coords.x)&&Number.isFinite(coords.y)?{
            x:.5+coords.x*rad/2+(cal.bcx||0)*scaleK/curSize,
            y:.5-coords.y*rad/2+(cal.bcy||0)*scaleK/curSize}:null;
          widokRzutow.zdjecia[n]={canvas,style,size:curSize,focus};
          zaloguj('zdjęcie rzutu '+(n+1)+': wybrana kamera; kadr '+style.width+' × '+style.height);widokRzutow.rev++;odswiezRzuty();
        }catch(e){zaloguj('zdjęcie rzutu: '+e.message);}
      },160);
    }
  }
  function wygladZdjecia(){
    const layer=typeof wrap!=='undefined'?wrap:null,hidden=layer?.style.display==='none';
    const old=hidden?{display:layer.style.display,visibility:layer.style.visibility}:null;
    try{
      // W trybie samych zdjęć wymuszamy pomiar ukrytej warstwy, bez jej wyświetlania.
      if(hidden)Object.assign(layer.style,{display:'block',visibility:'hidden'});
      const computed=getComputedStyle(video),style={};
      for(const k of ['width','height','left','top','transform','transformOrigin','objectFit'])style[k]=computed[k]||video.style[k];
      return style;
    }finally{if(old)Object.assign(layer.style,old);}
  }
  function blokadaZdjecia(count){return widokRzutow.wyjmowanie||(st.takeout&&count<3);}
  function celDwochT20(){
    if(!cal.zoomDwaT20||!lokalneRzuty()||!lotkaDostepna()||st.kolejkaSkonczona||st.legZamkniety||st.gameFinished||st.kolejka.bustPewny||st.takeout)return null;
    if(widokRzutow.rzuty.length!==2||!widokRzutow.rzuty.every(x=>x.label==='T20'))return null;
    if(st.numThrows!==null&&st.numThrows!==2)return null;
    // Cel obowiązuje tylko w tej kolejce, nawet gdy kolejny gracz też trafia 2×T20.
    return {txt:'T20',pt:segmentPoint('T20'),klucz:'two-T20|'+widokRzutow.key};
  }
  // promien domyslnie okragly; wewnatrz kadru ksztalt nadaje juz ramka,
  // wiec drugie kolo obcinaloby rogi przy zblizeniu.
  function dodajObrazRzutu(parent,shot,size,promien){
    const crop=el('div',{width:size+'px',height:size+'px',position:'relative',overflow:'hidden',borderRadius:promien==null?'50%':promien,background:'#0d0f14'});
    const inner=el('div',{position:'absolute',width:shot.size+'px',height:shot.size+'px',transformOrigin:'0 0',transform:'scale('+size/shot.size+')'});
    const c=document.createElement('canvas');c.width=shot.canvas.width;c.height=shot.canvas.height;c.getContext('2d').drawImage(shot.canvas,0,0);
    Object.assign(c.style,shot.style,{position:'absolute',display:'block',maxWidth:'none',maxHeight:'none',minWidth:'0',minHeight:'0',padding:'0',border:'none',boxSizing:'content-box'});inner.appendChild(c);crop.appendChild(inner);parent.appendChild(crop);return c;
  }
  // Zbliżenie kadru: 1 to cała tarcza w kadrze, więcej to widok przy lotce.
  // Trzymamy górną granicę, żeby dało się jeszcze rozpoznać sektor.
  function zoomKadru(){return Math.max(1,Math.min(5,Number(cal.rzutyZoom)||2.2));}
  // Obraz kamery nie wypełnia całego kwadratu nakładki — w sesji 19:37 wideo
  // miało 814×781 px w kwadracie 885 px, więc przy zbliżeniu na skraj tarczy
  // w kadrze pokazywał się pusty margines. Mierzymy realny prostokąt obrazu
  // (po transformacji perspektywy, bo to robi przeglądarka) i dociągamy
  // warstwę tak, żeby kadr został wypełniony.
  function dociagnijKadr(frame,layer,plotno){
    if(typeof frame.getBoundingClientRect!=='function'||typeof plotno.getBoundingClientRect!=='function')return;
    const f=frame.getBoundingClientRect(),o=plotno.getBoundingClientRect();
    if(!f.width||!f.height||!o.width||!o.height)return;
    const os=(fa,fb,oa,ob)=>ob-oa>=fb-fa?(oa>fa?fa-oa:ob<fb?fb-ob:0):(fa+fb)/2-(oa+ob)/2;
    const dx=os(f.left,f.right,o.left,o.right),dy=os(f.top,f.bottom,o.top,o.bottom);
    if(!dx&&!dy)return;
    layer.style.left=(parseFloat(layer.style.left)||0)+dx+'px';
    layer.style.top=(parseFloat(layer.style.top)||0)+dy+'px';
  }
  function dodajKadrRzutu(parent,shot,w,h,promien){
    const frame=el('div',{position:'relative',width:w+'px',height:h+'px',overflow:'hidden',borderRadius:promien||'9px',background:'#0d0f14'});
    const size=Math.round(Math.max(w,h)*zoomKadru()),f=shot.focus||{x:.5,y:.5};
    const layer=el('div',{position:'absolute',left:Math.max(w-size,Math.min(0,w/2-f.x*size))+'px',top:Math.max(h-size,Math.min(0,h/2-f.y*size))+'px'});
    // Dociągamy dopiero po wstawieniu paska do strony: element poza dokumentem
    // ma zerowy prostokąt, więc pomiar tutaj nic by nie dał.
    frame._warstwa=layer;frame._plotno=dodajObrazRzutu(layer,shot,size,'0');
    frame.appendChild(layer);parent.appendChild(frame);return frame;
  }
  function zamknijZdjecie(){if(podgladRzutu){const focus=podgladRzutu._powrot;podgladRzutu.remove();podgladRzutu=null;focus?.focus?.();}}
  function pokazZdjecie(i){
    const shot=widokRzutow.zdjecia[i];if(!shot)return;zamknijZdjecie();
    const box=el('div',{position:'fixed',inset:'0',zIndex:'10003',background:'rgba(4,8,15,.92)',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:'12px',color:C.tekst,font:C.font});
    box.dataset.adcamUi='shots';box.setAttribute('role','dialog');box.setAttribute('aria-modal','true');box.setAttribute('aria-label',t('Zdjęcie rzutu {a}',{a:i+1}));box._powrot=document.activeElement;podgladRzutu=box;
    box.appendChild(el('div',null,t('Rzut {a}: {b}',{a:i+1,b:widokRzutow.rzuty[i]?.label||'?'})));
    dodajObrazRzutu(box,shot,Math.max(80,Math.min(window.innerWidth-40,window.innerHeight-130,680)));
    const b=przycisk(box,t('Zamknij · Esc'),zamknijZdjecie);box.addEventListener('click',e=>{if(e.target===box)zamknijZdjecie();});
    box.addEventListener('keydown',e=>{if(e.key==='Tab'){e.preventDefault();b.focus();}});document.body.appendChild(box);b.focus();
  }
  // Tylko układ dwóch graczy: identyfikacja po dokładnej nazwie, bez klas motywu.
  let celChalkboard={key:'',at:0,node:null};
  // Dlaczego dok nie zostal znaleziony. W nagraniu z sesji 23:26 pasek spadal
  // na bok przez dlugie odcinki i z samego obrazu nie dalo sie orzec, ktory
  // warunek go odrzucil. Powod trafia do wiersza stanu i do dziennika przy
  // kazdej zmianie - bez tego kolejna sesja znowu bylaby zgadywanka.
  function powodDoku(p){
    if(p!==dokPowod){dokPowod=p;if(p!=='ok')zaloguj('chalkboard rywala niedostepny: '+p);}
    return null;
  }
  function chalkboardPrzeciwnika(){
    if(!daneSwieze())return powodDoku('dane nieswieze');
    if(st.players?.length!==2)return powodDoku('graczy='+(st.players?.length??'-'));
    const own=czyOnline()?mojIndeks():st.player;
    if(own!==0&&own!==1)return powodDoku('nieznany gracz lokalny ('+own+')');
    const name=String(st.players[1-own]?.name||'').trim();
    if(!name||name===String(st.players[own]?.name||'').trim())return powodDoku('nazwa rywala nieuzyteczna');
    const key=[st.matchId,own,name,window.innerWidth,window.innerHeight].join('|'),now=Date.now();
    if(key!==celChalkboard.key||now-celChalkboard.at>=1000||celChalkboard.node?.isConnected===false){
      celChalkboard={key,at:now,node:null};skanyLicznik++;
      let best=null,area=Infinity,etykiet=0,kart=0,odrzut='';
      for(const label of document.querySelectorAll('span,div,p,h1,h2,h3,a')){
        if(nasze(label)||(label.textContent||'').trim()!==name)continue;
        etykiet++;
        for(let card=label.parentElement,i=0;card&&i<9;card=card.parentElement,i++){
          const r=card.getBoundingClientRect();
          if(r.width<150||r.width>window.innerWidth*.44||r.height<280){odrzut=odrzut||'karta '+Math.round(r.width)+'x'+Math.round(r.height);continue;}
          kart++;
          for(const child of card.children){
            if(child.contains(label)||nasze(child))continue;
            const b=child.getBoundingClientRect(),lr=label.getBoundingClientRect();
            const zle=b.width<r.width*.7?'waskie dziecko':b.height<278?'niskie dziecko '+Math.round(b.height):
              b.top<=lr.bottom?'dziecko nad nazwa':(b.left<0||b.right>window.innerWidth||b.bottom>window.innerHeight||b.top<0)?
              'dziecko poza oknem':'';
            if(zle){odrzut=odrzut||zle;continue;}
            const a=b.width*b.height;if(a<area){best=child;area=a;}
          }
        }
      }
      celChalkboard.node=best;
      if(!best)powodDoku(etykiet?(kart?(odrzut||'zadne dziecko nie pasuje'):'karta odrzucona: '+odrzut):'nie znaleziono nazwy rywala');
    }
    const b=celChalkboard.node?.getBoundingClientRect();
    if(!b)return celChalkboard.node?powodDoku('wezel bez prostokata'):null;
    const zle=b.width<100?'szerokosc '+Math.round(b.width):b.height<278?'wysokosc '+Math.round(b.height):
      (b.top<0||b.bottom>window.innerHeight)?'poza oknem w pionie':(b.left<0||b.right>window.innerWidth)?'poza oknem w poziomie':'';
    if(zle)return powodDoku('prostokat: '+zle);
    powodDoku('ok');return b;
  }
  let przesuwanieRzutow=null;
  // Kadry na chalkboardzie zawsze jeden pod drugim — tak zdecydował autor po
  // teście w grze. Obszar rywala jest z góry dany, więc jedyne miejsce, jakie da
  // się jeszcze odzyskać, to wiersz podpisu: od 1.1.6 podpis leży NA kadrze,
  // a nie pod nim, i o tyle kadr jest wyższy. Rachunek: ramka 2 px, padding
  // 2×4 px, uchwyt 22+6 px, dwie przerwy po 6 px.
  function ukladKadrow(dock){
    const w=Math.max(1,Math.floor(dock.width-10));
    const wys=Math.max(1,Math.floor((dock.height-50)/3));
    return {kolumna:true,w,h:wys,bok:Math.min(w,wys)};
  }
  // Punkty rywala mają zostać widoczne, reszta jego kolumny już nie musi:
  // pasek zaczyna się pod jego wynikiem i schodzi do samego dołu okna.
  function obszarDoku(dock){
    if(!dock)return null;
    const dol=Math.max(dock.top+dock.height,window.innerHeight-8);
    return {left:dock.left,top:dock.top,width:dock.width,height:Math.max(120,dol-dock.top)};
  }
  function graniceZdjec(w,h){return {x:Math.max(8,window.innerWidth-w-8),y:Math.max(8,window.innerHeight-h-8)};}
  function ograniczZdjecia(x,y,w,h){const m=graniceZdjec(w,h);return {x:Math.max(8,Math.min(m.x,x)),y:Math.max(8,Math.min(m.y,y))};}
  function ustawPozycjeZdjec(side,size,dock=null){
    if(!dock)Object.assign(pasekRzutow.style,{width:'auto',height:'auto',boxSizing:'content-box'});
    const r=pasekRzutow.getBoundingClientRect?.();
    const w=r?.width||(side?size:size*3+16)+18,h=r?.height||(side?(size+24)*3+16:size+24)+46;
    if(dock){
      Object.assign(pasekRzutow.style,{left:dock.left+'px',top:dock.top+'px',width:dock.width+'px',height:dock.height+'px',boxSizing:'border-box',right:'auto',bottom:'auto',transform:'none'});return;
    }
    const m=graniceZdjec(w,h),bar=pasekAutodarts();
    const dol=bar?bar.dol+bar.wys+12:110;
    const own=Number.isFinite(cal.rzutyX)&&cal.rzutyX>=0&&Number.isFinite(cal.rzutyY)&&cal.rzutyY>=0;
    const p=ograniczZdjecia(own?8+cal.rzutyX*(m.x-8):side?12:(window.innerWidth-w)/2,
      own?8+cal.rzutyY*(m.y-8):side?(window.innerHeight-h)/2:window.innerHeight-dol-h,w,h);
    Object.assign(pasekRzutow.style,{left:p.x+'px',top:p.y+'px',right:'auto',bottom:'auto',transform:'none'});
  }
  function zapamietajPozycjeZdjec(x,y,w,h){
    if(cal.rzutyPozycja==='opponent'){cal.rzutyPozycja='side';rysRzutyKlucz='';}
    const p=ograniczZdjecia(x,y,w,h),m=graniceZdjec(w,h);
    cal.rzutyX=(p.x-8)/Math.max(1,m.x-8);cal.rzutyY=(p.y-8)/Math.max(1,m.y-8);
    Object.assign(pasekRzutow.style,{left:p.x+'px',top:p.y+'px'});
  }
  function podepnijPrzesuwanieZdjec(b){
    b.addEventListener('pointerdown',e=>{
      if(e.button!==0)return;
      const r=pasekRzutow.getBoundingClientRect();
      przesuwanieRzutow={id:e.pointerId,x:e.clientX,y:e.clientY,left:r.left,top:r.top,w:r.width,h:r.height};
      b.setPointerCapture(e.pointerId);b.focus();e.preventDefault();e.stopPropagation();
    });
    b.addEventListener('pointermove',e=>{
      const d=przesuwanieRzutow;if(!d||d.id!==e.pointerId)return;
      zapamietajPozycjeZdjec(d.left+e.clientX-d.x,d.top+e.clientY-d.y,d.w,d.h);e.stopPropagation();
    });
    const koniec=e=>{if(przesuwanieRzutow?.id!==e.pointerId)return;przesuwanieRzutow=null;save();};
    for(const name of ['pointerup','pointercancel','lostpointercapture'])b.addEventListener(name,koniec);
    b.addEventListener('keydown',e=>{
      if(!['ArrowLeft','ArrowRight','ArrowUp','ArrowDown'].includes(e.key))return;
      const r=pasekRzutow.getBoundingClientRect(),step=e.shiftKey?1:10;
      zapamietajPozycjeZdjec(r.left+(e.key==='ArrowLeft'?-step:e.key==='ArrowRight'?step:0),r.top+(e.key==='ArrowUp'?-step:e.key==='ArrowDown'?step:0),r.width,r.height);
      save();e.preventDefault();e.stopPropagation();
    });
  }
  function odswiezRzuty(){
    const mode=trybWidoku();
    if(widokRzutow.key&&!daneSwieze())resetRzutow();
    const show=mode!=='live'&&visible&&camOk&&wMeczu()&&lokalneRzuty()&&!inReplay&&!kreator&&!oknoOtwarte&&!labOtwarte&&!oknoAplikacji;
    if(!show){przesuwanieRzutow=null;if(pasekRzutow)pasekRzutow.style.display='none';zamknijZdjecie();return;}
    if(!pasekRzutow){pasekRzutow=el('section',{position:'fixed',zIndex:'9002',padding:'8px',background:'rgba(12,22,32,.96)',border:'1px solid #304355',borderRadius:'14px',color:C.tekst,font:C.font});pasekRzutow.dataset.adcamUi='shots';document.body.appendChild(pasekRzutow);}
    pasekRzutow.style.display='block';
    const dock=obszarDoku(cal.rzutyPozycja==='opponent'?chalkboardPrzeciwnika():null);
    const uklad=dock?ukladKadrow(dock):null;
    const side=dock?uklad.kolumna:(['side','opponent'].includes(cal.rzutyPozycja)&&window.innerHeight>=360);
    const zadany=Math.max(48,Number(cal.rzutyRozmiar)||200);
    const size=Math.max(48,Math.floor(Math.min(zadany,dock?uklad.bok:side?(window.innerHeight-190)/3:(window.innerWidth-72)/3)));
    if(przesuwanieRzutow)return;
    // W doku kadry wypełniają obszar — suwak rozmiaru dotyczy dołu i boku.
    const tileW=dock?uklad.w:size,tileH=dock?uklad.h:size;
    pasekRzutow.style.padding=dock?'4px':'8px';
    const key=[widokRzutow.rev,mode,side,size,tileW,tileH,!!dock,zoomKadru(),jezyk()].join('|');if(key===rysRzutyKlucz){ustawPozycjeZdjec(side,size,dock);return;}rysRzutyKlucz=key;
    pasekRzutow.textContent='';pasekRzutow.setAttribute('aria-label',t('Trzy ostatnie rzuty'));
    const uchwyt=el('button',{display:'block',width:'100%',height:'22px',padding:'0',margin:'0 0 6px',border:'none',borderRadius:'5px',background:'transparent',color:C.slaby,cursor:'grab',touchAction:'none',font:'inherit'},'⋮⋮');
    uchwyt.type='button';uchwyt.dataset.adcamDrag='shots';uchwyt.setAttribute('aria-label',t('Przesuń zdjęcia'));uchwyt.title=t('Przeciągnij lub użyj strzałek');
    podepnijPrzesuwanieZdjec(uchwyt);pasekRzutow.appendChild(uchwyt);
    const row=el('div',{display:'flex',flexDirection:side?'column':'row',gap:dock?'6px':'8px'});pasekRzutow.appendChild(row);
    const kadry=[];
    for(let i=0;i<3;i++){
      const shot=widokRzutow.zdjecia[i],r=widokRzutow.rzuty[i];
      const b=el('button',{padding:'0',border:'none',background:'transparent',color:C.tekst,width:tileW+'px',fontSize:'12px'},'');b.type='button';b.disabled=!shot;
      b.setAttribute('aria-label',t('Rzut {a}: {b}',{a:i+1,b:r?.label||t('Oczekuję na rzut')}));
      const ksztalt=dock?'9px':'50%';
      const ramka=shot?dodajKadrRzutu(b,shot,tileW,tileH,ksztalt)
        :b.appendChild(el('div',{position:'relative',width:tileW+'px',height:tileH+'px',borderRadius:ksztalt,background:'#1b2937',display:'flex',alignItems:'center',justifyContent:'center',boxSizing:'border-box',padding:'8px'},t(r?'Brak zdjęcia':'Oczekuję na rzut')));
      // W doku podpis leży NA kadrze, żeby nie zjadał wysokości obrazu.
      const podpis=dock
        ?el('div',{position:'absolute',left:'0',right:'0',bottom:'0',padding:'1px 6px 2px',whiteSpace:'nowrap',textAlign:'center',pointerEvents:'none',background:'linear-gradient(to top,rgba(4,8,15,.86),rgba(4,8,15,0))'},(i+1)+' · '+(r?.label||'—'))
        :el('div',{marginTop:'5px',whiteSpace:'nowrap'},(i+1)+' · '+(r?.label||'—'));
      (dock?ramka:b).appendChild(podpis);
      b.addEventListener('click',()=>pokazZdjecie(i));row.appendChild(b);
      if(ramka._plotno)kadry.push(ramka);
    }
    ustawPozycjeZdjec(side,size,dock);
    for(const ramka of kadry)dociagnijKadr(ramka,ramka._warstwa,ramka._plotno);
  }
  function wyborWidoku(parent){
    const select=el('select',{width:'100%',padding:'8px',margin:'6px 0',color:C.tekst,background:C.tlo2,border:'1px solid '+C.ramka,borderRadius:'8px'});select.setAttribute('aria-label',t('Widok tarczy'));
    for(const [value,label]of [['live','Na żywo'],['shots','Trzy rzuty'],['both','Na żywo + rzuty']]){const o=el('option',null,t(label));o.value=value;select.appendChild(o);}
    select.value=trybWidoku();select.addEventListener('change',()=>ustawWidokRzutow(select.value));select.addEventListener('keydown',e=>e.stopPropagation());parent.appendChild(select);
    parent._odswiez=parent._odswiez||[];parent._odswiez.push(()=>{select.value=trybWidoku();});return select;
  }
  const C = {
  ...
  function buildHud(){
    ...
    wyborWidokuPanel=wyborWidoku(panelBody);
  ...
  // Plywajacy przycisk kamery. Sluzy do jednego: schowac obraz na czas
  // poprawiania wyniku palcem, bez siegania po klawiature. Nie dotyka kamery,
  // nagrywania ani kalibracji - przelacza dokladnie ten sam stan co skrot K,
  // przez ustawWidocznoscKamery, zeby nie powstal drugi mechanizm widocznosci.
  function budujPrzyciskKamery(){
    if(przyciskKamery)return przyciskKamery;
    // Prawa krawedz, nad srodkiem pionu: tam nie ma ani paska Autodarts z Next,
    // ani kart graczy, ani naszej pastylki w lewym dolnym rogu.
    const b=el('button',{position:'fixed',right:'12px',top:'50%',transform:'translateY(-50%)',
      zIndex:'10004',width:'48px',height:'48px',minWidth:'44px',minHeight:'44px',
      display:'none',alignItems:'center',justifyContent:'center',padding:'0',
      borderRadius:'14px',border:'1px solid '+C.ramka,background:'rgba(16,25,35,.92)',
      color:C.tekst,font:C.font,fontSize:'20px',lineHeight:'1',cursor:'pointer',
      touchAction:'manipulation',userSelect:'none',boxShadow:'0 10px 30px #0007'});
    b.type='button';b.dataset.adcamUi='camera-toggle';
    // Jedno zrodlo zdarzenia. Pointerdown tylko blokuje przeciaganie strony pod
    // spodem; przelacza wylacznie click, wiec dotyk nie przelacza dwa razy.
    b.addEventListener('pointerdown',e=>{e.preventDefault();e.stopPropagation();});
    b.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();ustawWidocznoscKamery(!visible);});
    document.body.appendChild(b);przyciskKamery=b;return b;
  }
  function odswiezPrzyciskKamery(){
    if(!cal.przyciskKamery){if(przyciskKamery)przyciskKamery.style.display='none';return;}
    const b=budujPrzyciskKamery();
    // Chowamy pod modalem Autodarts i kreatorem tak samo jak pastylke,
    // zeby nie zaslonic ich kontrolek.
    b.style.display=!oknoAplikacji&&!kreator?'flex':'none';
    b.textContent=visible?'\u{1F4F7}':'\u{1F576}';
    const opis=t(visible?'Ukryj obraz kamery':'Pokaż obraz kamery');
    b.setAttribute('aria-label',opis);b.title=opis;
    b.setAttribute('aria-pressed',visible?'false':'true');
  }
  function drawHud(){
    odswiezFormatNagrania?.();
    odswiezPrzyciskKamery();
    if(!hud||!panelStatus)return;
    if(wyborWidokuPanel)wyborWidokuPanel.value=trybWidoku();
  ...
  function poprawneUstawienia(o){
    ...
    if(!['live','shots','both'].includes(c.widokRzutow)||!['bottom','side','opponent'].includes(c.rzutyPozycja))throw Error('Błędny widok tarczy');
    if(o._migr113!==true){c.rzutyRozmiar=Math.max(140,Number(c.rzutyRozmiar)||140);c._migr113=true;}
    if(o._migr116!==true){c.rzutyRozmiar=Math.max(200,Number(c.rzutyRozmiar)||200);c.rzutyZoom=Number(c.rzutyZoom)||2.2;c._migr116=true;}
    if(o._migr111!==true){c.zoomDwaT20=true;c._migr111=true;}
    for(const [key,min,max] of [['rzutyRozmiar',48,600],['rzutyZoom',1,5],['rzutyX',-1,1],['rzutyY',-1,1]])if(c[key]<min||c[key]>max)throw Error('Wartość poza zakresem: '+key);
  ...
  function przebudujUI() {
    ...
    zamknijZdjecie();pasekRzutow?.remove();pasekRzutow=null;rysRzutyKlucz='';
    for (const n of [hud, okno, pigulka, labOkno, pasekPow, pytanie, przyciskKamery]) { try { if (n && n.remove) { n._sprzataj?.(); n._resizeObserver?.disconnect(); n.remove(); } } catch (e) {} }
    hud = null; okno = null; pigulka = null; labOkno = null; pasekPow=null;pytanie=null;przyciskKamery=null;
  ...
  function budujKarteObraz(k){
    ...
    naglowek(k,t('Widok tarczy'));
    wyborWidoku(k);
    podpowiedz(k,t('Trzy rzuty zachowują zdjęcia po kolejnych lotkach z wybranej kamery. Kliknij zdjęcie, aby je powiększyć. Nowa kolejka czyści zdjęcia.'));
    const pozycja=el('select',{width:'100%',padding:'8px',color:C.tekst,background:C.tlo2});
    pozycja.setAttribute('aria-label',t('Położenie zdjęć'));
    for(const [value,label] of [['bottom','Zdjęcia na dole'],['side','Zdjęcia z boku'],['opponent','Na chalkboardzie przeciwnika']]){const o=el('option',null,t(label));o.value=value;pozycja.appendChild(o);}
    pozycja.addEventListener('change',()=>{cal.rzutyPozycja=pozycja.value;cal.rzutyX=-1;cal.rzutyY=-1;save();rysRzutyKlucz='';odswiezRzuty();});
    pozycja.addEventListener('keydown',e=>e.stopPropagation());k.appendChild(pozycja);
    k._odswiez.push(()=>{pozycja.value=cal.rzutyPozycja;pozycja.disabled=trybWidoku()==='live';});
    podpowiedz(k,t('Na chalkboardzie trzy duże kadry pokazują okolice rzutów pod punktami rywala, aż do dołu okna. Kliknij kadr, aby zobaczyć całą tarczę. Jeśli nie rozpoznam miejsca, zdjęcia pojawią się z boku.'));
    suwak(k,t('Rozmiar zdjęć'),'rzutyRozmiar',48,600,4,' px');
    suwak(k,t('Zbliżenie zdjęć'),'rzutyZoom',1,5,.1,'×');
    podpowiedz(k,t('Zbliżenie 1× to cała tarcza w kadrze; wyżej kadr podjeżdża do lotki. Na chalkboardzie kadry same wypełniają kolumnę rywala aż do dołu okna, więc suwak rozmiaru dotyczy układu dolnego i bocznego.'));
    // Kadr celuje w lotkę po kole tarczy z kalibracji — tym samym, które rysuje
    // siatka pomocnicza. Gdy koło nie pokrywa się z tarczą na obrazie, kadry
    // systematycznie mijają lotkę, więc kontrola jest tutaj, przy zdjęciach.
    podpowiedz(k,t('Kadr celuje w lotkę według koła tarczy z kalibracji. Włącz siatkę pomocniczą: jeśli narysowany okrąg nie pokrywa się z zewnętrzną krawędzią podwójnego pierścienia, kadry będą mijać lotkę.'));
    przycisk(k,t('Siatka pomocnicza'),()=>{showGuide=!showGuide;});
    const kolo=el('fieldset');k.appendChild(kolo);
    suwak(kolo,t('Promień tarczy'),'brad',.6,1.5,.01);
    k._odswiez=k._odswiez||[];k._odswiez.push(()=>{
      const auto=!!aktywnaHomografia();
      kolo.disabled=auto;kolo.style.opacity=auto?'.5':'1';
      for(const f of kolo._odswiez||[])f();
    });
    podpowiedz(k,t('Dopasowanie pięciu punktów ustawia to koło samo i dokładnie. Suwak jest dla obrazu bez dopasowania punktowego.'));
    podpowiedz(k,t('Przeciągnij uchwyt nad zdjęciami, aby przesunąć pasek. Po kliknięciu uchwytu możesz też użyć strzałek.'));
    przycisk(k,t('Przywróć położenie zdjęć'),()=>{cal.rzutyX=-1;cal.rzutyY=-1;save();odswiezRzuty();});
    naglowek(k,t('Dopasowanie do tarczy'));
    const powod=el('div',{display:'none',margin:'6px 0',padding:'8px 10px',borderRadius:'8px',
      background:'#3a2a12',border:'1px solid #6b4a17',color:'#f0d9a8',fontSize:'12px'});
    powod.setAttribute('role','status');k.appendChild(powod);
    k._odswiez=k._odswiez||[];k._odswiez.push(()=>{
      // Powod liczymy z biezacego stanu, nie tylko z tej sesji, w ktorej
      // dopasowanie samo sie wylaczylo - inaczej po restarcie panel milczy.
      const inna=!!cal.punktowa&&cal.punktowa.deviceId&&kameraDeviceId&&cal.punktowa.deviceId!==kameraDeviceId;
      const powodTeraz=powodKalibracji||(inna?'Dopasowanie zapisano dla innej kamery':'');
      const jest=!!cal.punktowa&&!cal.punktowa.enabled&&!!powodTeraz;
      powod.style.display=jest?'block':'none';
      powod.textContent=jest?t('Dopasowanie punktowe wyłączone: {a}. Bez niego kadry celują po ręcznie ustawionym kole tarczy. Wykonaj dopasowanie ponownie.',{a:t(powodTeraz)}):'';
    });
  ...
  function budujKartePreferencje(k) {
    ...
    naglowek(k,t('Ułatwienia dla touchscreenów'));
    przelacznik(k,t('Pływający przycisk kamery'),'przyciskKamery',
      t('Pokazuje przy tarczy przycisk, którym można szybko ukryć lub pokazać obraz kamery podczas poprawiania wyniku.'));
    podpowiedz(k,t('To samo, co skrót K, tylko bez klawiatury. Ukrywa wyłącznie obraz — kamera, nagrywanie i kalibracja pracują dalej.'));
  ...
  function budujKarteZoom(k) {
    ...
    przelacznik(g,t('Zoom po dwóch T20'),'zoomDwaT20',
      t('Po dwóch T20 w tej samej kolejce przybliża T20 przed trzecią lotką. Jeśli możesz zamknąć lega, pierwszeństwo ma pole na zamknięcie.'));
  ...
  function place() {
    ...
    curSize = size;
    Object.assign(wrap.style, {
      display: trybWidoku()==='shots'&&!oknoOtwarte&&!showGuide ? 'none' : 'block', width: `${size}px`, height: `${size}px`,
  ...
  function applyZoom() {
    ...
    else if (cal.zoomWl) target = checkoutTarget() || celDwochT20();
  ...
  async function startCamera() {
    ...
    for (const t of CONFIG.tryby.filter(x => x.w <= cal.maxSzer)) {
      ...
      try {
        ...
        await video.play();
        if(attempt!==cameraAttempt)return;
        if(cal.punktowa&&Math.abs(video.videoWidth/video.videoHeight-cal.punktowa.ratio)>.02){
          cal.punktowa.enabled=false;save();
          powodKalibracji='Zmieniono proporcje obrazu kamery';
          zaloguj('dopasowanie punktowe wyłączone: inne proporcje obrazu');
          toast(t("Zmieniono proporcje kamery — wykonaj kalibrację ponownie"));}
        mierzKlatki();
        const g = st.getVideoTracks()[0].getSettings();
        kameraDeviceId = g.deviceId || '';
        if(cal.punktowa?.enabled && cal.punktowa.deviceId && cal.punktowa.deviceId!==g.deviceId){
          cal.punktowa.enabled=false;save();
          powodKalibracji='Dopasowanie zapisano dla innej kamery';
          zaloguj('dopasowanie punktowe wyłączone: kalibracja należy do innej kamery');
          toast(t('Inna kamera — wykonaj dopasowanie obrazu'));
        }
        if(cal.punktowa?.enabled)powodKalibracji='';
  ...
  function onKey(e) {
    if(e.target?.dataset?.adcamDrag==='shots'&&e.key.startsWith('Arrow'))return;
    if(podgladRzutu){if(e.key==='Escape'){zjedz(e);zamknijZdjecie();}return;}
  ...
  (function loop() {
    try {
      ...
      if (t - ostatnieRys > 500) {
        ostatnieRys = t;
        drawHud(); odswiezTryb(); drawDiagOkno(); odswiezRzuty();
      }
